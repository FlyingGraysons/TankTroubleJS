...
var isReachable = function(tile, direction, rows, columns){
	var myX = tile.position[0]+changes[direction][0];
	var myY = tile.position[1]+changes[direction][1];
	if(myX >= 0 && myX < columns && myY >= 0 && myY < rows){
...
function createMap(map_rows, map_columns){
	var MAP_ROWS = map_rows;
	var MAP_COLUMNS = map_columns;
	var map =[];
